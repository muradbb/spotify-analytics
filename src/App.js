...
function FavArtist(props) {
 return ( <
  ...
  p > Genres: {
   props.artist.genres.length > 0 ? props.artist.genres.map(
 ...
 return ( <
   ...
   function UserTrends(props) {
    ...
    return ( <
     ...
     <
     h3 > This is a Pie Chart
     for the genres you listen to < /h3> <
     Chart width = {
      '800px'
     }
     height = {
     ...
     useEffect(() => {
      ...
      Promise.all([
       ...
       .then(([userData, playlistData, favArtistDataLT, favArtistDataMT, favArtistDataST, favTracksLT, favTracksMT, favTracksST]) => {
        return Promise.all([userData.json(), playlistData.json(), favArtistDataLT.json(), favArtistDataMT.json(), favArtistDataST.json(), favTracksLT.json(), favTracksMT.json(), favTracksST.json()])
       })
       .then(([userData, playlistData, favArtistDataLT, favArtistDataMT, favArtistDataST, favTracksLT, favTracksMT, favTracksST]) =>
        setServerData({
         user: {
           ...
           serverData.user.favArtists.mediumTerm.slice(0, 5).map(artist =>
            ...
            h2 > Your recent time favourite < /h2> {
             serverData.user.favArtists.shortTerm.slice(0, 5).map(artist =>
              ...
              <
              h2 > These are a list of songs that were your favourite over a certian period of time < /h2> <
