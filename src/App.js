  ...
  return (
    <div style={{ color: "#FFA07A" }}>
      <h1 style={{ textAlign: "center" }}>
        ...
        playlists which is a total of {totalSongs} songs.
      </h1>
    </div>
  );
}
...
function FavArtist(props) {
  let genreString = props.artist.genres
    .slice(0, 5)
    .map(
      (genre, index) =>
        " " +
...
function RelatedArtists(props) {
  let allArtists = props.artists.longTerm
    .slice(0, 5)
    .concat(
      props.artists.mediumTerm
        .slice(0, 5)
        .concat(props.artists.shortTerm.slice(0, 5))
    );

  function searchedArtist(title) {
    return allArtists.find(
      (artist) => artist.name.toLowerCase() === title.toLowerCase()
    );
  }
  ...
  useEffect(() => {
    ...
    if (typeof relatedArtists !== "undefined")
      Promise.all([
        ...
        .then(([firstData, secondData, thirdData, fourthData, fifthData]) => {
          return Promise.all([
            firstData.json(),
            secondData.json(),
            thirdData.json(),
  ...
  function handleCreateSubmit(e) {
    ...
    let allURIs = relatedArtistTracks.firstArtistsTracks
      .slice(0, 5)
      .map((track) => {
        return track.uri;
      })
      ...
      .concat(
        "," +
          relatedArtistTracks.secondArtistsTracks
            .slice(0, 5)
            .map((track) => {
              return track.uri;
            })
      ...
      .concat(
        "," +
          relatedArtistTracks.thirdArtistsTracks
            .slice(0, 5)
            .map((track) => {
              return track.uri;
            })
      ...
      .concat(
        "," +
          relatedArtistTracks.fourthArtistsTracks
            .slice(0, 5)
            .map((track) => {
              return track.uri;
            })
      ...
      .concat(
        "," +
          relatedArtistTracks.fifthArtistsTracks
            .slice(0, 5)
            .map((track) => {
              return track.uri;
            })
  ...
  return (
    <div style={{ backgroundColor: "", margin: "auto", marginBottom: "60px" }}>
      <h2 style={{ textAlign: "center" }}>
        Enter names of one of your favourite artists to get related artists to
        them
      </h2>
      <form
        ...
        <div>
          ...
          <div>
            <p style={{ fontSize: "20px", textAlign: "center" }}>
              {
                "If you wish you can create a playlist with these artists' top tracks. Press Create if you want to proceed"
              }
            </p>
...
function AudioFeatureTrend(props) {
  ...
  useEffect(() => {
    ...
    fetch("https://api.spotify.com/v1/audio-features?ids=" + allSongIds, {
      ...
      .then((audioFeatureData) => audioFeatureData.json())
      .then((audioFeatureData) => {
        setAudioFeatures(
          audioFeatureData.audio_features.map((item) => {
            return {
  ...
  if (typeof audioFeatures !== "undefined") {
    ...
    let allInstrumentalness = audioFeatures.reduce(
      (instrumentalness, eachTrack) => {
  ...
  function speechinessDecision(speechiness) {
    let basicResponse =
      "The average speechiness for your top tracks suggests that you listen to tracks ";
    if (speechiness <= 0.33) {
  ...
  return (
    <div className="AudioFeatures">
      ...
      >
        Audio trends for your top 50 tracks
      </h2>
      {typeof audioFeatures !== "undefined" ? (
        <div>
          <p>
            {acousticnessDecision(avgAcousticness)} Your average acousticness
            score is {Math.floor(avgAcousticness * 1000) / 1000} highest
            possible point being 1.0
          </p>
          <p>
            {danceabilityDecision(avgDanceability)} Your average danceability
            score is {Math.floor(avgDanceability * 1000) / 1000} highest
            possible point being possible 1.0. The higher the value, the easier
            it is to dance to the song.
          </p>
          <p>
            Average duration for your favourite songs is{" "}
            {toHumanTime(avgDuration)}
          </p>
          <p>
            {energyDecision(avgEnergy)} Your average energy score is{" "}
            {Math.floor(avgEnergy * 1000) / 1000} highest possible point being
            1.0.
          </p>
          <p>
            {speechinessDecision(avgSpeechiness)} Your average speechiness score
            is {Math.floor(avgSpeechiness * 1000) / 1000}
          </p>
          <p>Average BPM for you favourite songs is {Math.floor(avgTempo)}</p>
          <p>
            Your average valence score is {Math.floor(avgValence * 1000) / 1000}
            . Tracks with high valence sound more positive (happy, cheerful,
            euphoric), while tracks with low valence sound more negative (sad,
            depressed, angry).
          </p>
        </div>
      ) : (
...
function PlaylistCreator(props) {
  ...
  let allSongURIsLT = props.tracks.longTerm
    .map((track, index) => {
      return track.uri;
    })
    .join();
  let allSongURIsMT = props.tracks.mediumTerm
    .map((track, index) => {
      return track.uri;
    })
    .join();
  let allSongURIsST = props.tracks.shortTerm
    .map((track, index) => {
      return track.uri;
    })
    .join();
  ...
  useEffect(() => {
    ...
    if (typeof playlistName !== "undefined")
      fetch("https://api.spotify.com/v1/users/" + props.userID + "/playlists", {
        ...
        .then((playlistData) => playlistData.json())
        .then((playlistData) => {
          fetch(
            "https://api.spotify.com/v1/playlists/" +
              playlistData.id +
  ...
  return (
    <div>
      <h2>
        In this section you can create playlists with your top 50 tracks from
      ...
      <div className="ChoiceBox">
        ...
        >
          Choose the preferred time period:{" "}
        </p>
...
function RecentlyPlayed(props) {
  ...
  function wordCutter(string) {
    return string.length > 16 ? string.substr(0, 13).concat("...") : string;
  }
    ...
    return (
      ...
      >
        <img
          ...
          height="100"
          width="100"
          style={{ marginLeft: "2px", padding: "0", marginRight: "2px" }}
        />
  ...
  return recentSongs ? (
    <div>
      ...
      <p style={{ fontSize: "20px", margin: "5px", textAlign: "center" }}>
        Ever listened to a song on spotify then forgot the name? Worry not! Just
        enter the number of recently played songs you would like to see.
      </p>
      <form
        onSubmit={handleSubmit}
  ...
  return (
    <div style={{ border: "3px solid #fe9b1d", margin: "2%" }}>
      <div style={{ marginLeft: "10px", marginBottom: "10px" }}>
        <h3>
          By pressing create you can make a playlist from your top {"artists'"}{" "}
          top tracks
        </h3>
        <form onSubmit={handleSubmit}>
          <p>How many of your top artists (max:40)</p>
          <input
            type="text"
            value={artistLimit}
          ...
          />
          <p>How many of each artists top songs (max:10)</p>
          <input
            type="text"
            value={trackLimit}
      ...
      {serverData ? (
        <div>
          ...
          <div className="FavArtists">
            ...
            <TopArtistPlaylistCreator
              artists={serverData.user.favArtists.longTerm}
            ...
            <TopArtistPlaylistCreator
              artists={serverData.user.favArtists.mediumTerm}
              userID={serverData.user.id}
            />
            ...
            <TopArtistPlaylistCreator
              artists={serverData.user.favArtists.shortTerm}
              userID={serverData.user.id}
            />
          </div>
          ...
          />
          <RelatedArtists
            artists={serverData.user.favArtists}
            userID={serverData.user.id}
          />
          <AudioFeatureTrend tracks={serverData.user.favTracks.longTerm} />
          <PlaylistCreator
            userID={serverData.user.id}
            tracks={serverData.user.favTracks}
          />
          <RecentlyPlayed />
        </div>
      ) : (
        ...
        <div style={{textAlign:'center'}}>
          ...
          <h3>This app is a tool for analyzing you spotify data and showing you useful insights</h3>
        </div>
        </div>
      )
